...
from CDTR.data.dataset import KGSeqDataset, SequentialDataset
from CDTR.data.interaction import Interaction
from CDTR.sampler import SeqSampler
from CDTR.utils.enum_type import FeatureType
...
class DIENDataset(SequentialDataset):
    """:class:`DIENDataset` is based on :class:`~CDTR.data.dataset.sequential_dataset.SequentialDataset`.
    ...
    Attributes:
        augmentation (bool): Whether the interactions should be augmented in CDTR.
        seq_sample (CDTR.sampler.SeqSampler): A sampler used to sample negative item sequence.

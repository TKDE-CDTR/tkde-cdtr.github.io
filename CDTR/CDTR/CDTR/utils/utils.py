...
from CDTR.utils.enum_type import ModelType
...
def get_model(model_name):
    ...
    for submodule in model_submodule:
        module_path = '.'.join(['CDTR.model', submodule, model_file_name])
...
def get_trainer(model_type, model_name,task=None,robust=None):
    ...
    if task!=None:
        if task.lower() == 'dr_mf':
            if robust == True:
                return getattr(importlib.import_module('CDTR.trainer'), 'MFRDDRTrainer')
            return getattr(importlib.import_module('CDTR.trainer'), 'MFDRTrainer')

        if task.lower()=='ips':
            if robust==True:
                return getattr(importlib.import_module('CDTR.trainer'), 'UMIPSTrainer')
            return getattr(importlib.import_module('CDTR.trainer'), 'IPSTrainer')
        if task.lower()=='dr':
            if robust==True:
                return getattr(importlib.import_module('CDTR.trainer'), 'UMDRTrainer')
            return getattr(importlib.import_module('CDTR.trainer'), 'DRTrainer')
        if task.lower() == 'dips':
            return getattr(importlib.import_module('CDTR.trainer'), 'DIPSTrainer')
    try:
        return getattr(importlib.import_module('CDTR.trainer'), model_name + 'Trainer')
    except AttributeError:
        if model_type == ModelType.KNOWLEDGE:
            return getattr(importlib.import_module('CDTR.trainer'), 'KGTrainer')
        elif model_type == ModelType.TRADITIONAL:
            return getattr(importlib.import_module('CDTR.trainer'), 'TraditionalTrainer')
        else:
            return getattr(importlib.import_module('CDTR.trainer'), 'Trainer')

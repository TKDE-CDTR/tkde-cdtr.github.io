...
def cluster_info(module_name):
    ...
    Note:
        For ``metric_type``: in current CDTR, all the "grouped-score" metrics are ranking-based and all the
        "non-grouped-score" metrics are value-based. To keep with our paper, we adopted the more formal terms:
        ``RANKING`` and ``VALUE``.

    Args:
        module_name (str): the name of module ``CDTR.evaluator.metrics``.
...
metric_module_name = 'CDTR.evaluator.metrics'

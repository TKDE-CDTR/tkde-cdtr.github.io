...
from CDTR.evaluator import metric_types, smaller_metrics
from CDTR.utils import get_model, Enum, EvaluatorType, ModelType, InputType, \
...
class Config(object):
    ...
    Configurator module will first load the default parameters from the fixed properties in CDTR and then
    ...
    def _load_cmd_line(self):
        ...
        if len(unrecognized_args) > 0:
            logger = getLogger()
            logger.warning('command line args [{}] will not be used in CDTR'.format(' '.join(unrecognized_args)))
    ...
    def _load_internal_config_dict(self, model, model_class, dataset):
        ...
        if self.internal_config_dict['MODEL_TYPE'] == ModelType.GENERAL:
            ...
            if model in ['DIN', 'DIEN']:
                ...
                if dataset == 'ml-100k_origin' and model in ['GRU4RecF', 'SASRecF', 'FDSA', 'S3Rec']:
    ...
    def _set_default_parameters(self):
        ...
        if self.dataset == 'ml-100k_origin':
            current_path = os.path.dirname(os.path.realpath(__file__))
